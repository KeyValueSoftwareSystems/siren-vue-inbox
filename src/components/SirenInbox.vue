  ...
  <div :class="!windowViewOnly && 'siren-sdk-inbox-container'" ref="notificationRef">
    <div v-if="!windowViewOnly" ref="iconRef">
      <NotificationIcon :handleNotification="handleNotification"
        :badgeType="showNotifications ? BadgeType.NONE : BadgeType.DEFAULT" :darkMode="darkMode" :styles="styles" />
    </div>
    <div v-if="showNotifications || windowViewOnly" :style="{
    ...styles.container,
    ...(!windowViewOnly && styles.windowShadow),
    width: windowViewOnly || windowWidth < 500 ? '100%' : styles?.container?.width,
    position: windowViewOnly || windowWidth < 500 ? 'initial' : 'absolute',
    ...modalPosition,
  }">
      <SirenPanel :hideHeader="hideHeader" :title="title" :onNotificationCardClick="onNotificationCardClick"
        :darkMode="darkMode" :styles="styles" :windowViewOnly="windowViewOnly" :showNotifications="showNotifications"
        :noOfNotificationsPerFetch="noOfNotificationsPerFetch" :hideClearAll="hideClearAll">
...
import { ref, watch, defineProps, withDefaults, computed, onMounted, onBeforeUnmount } from 'vue';

import { SirenProps } from '../types';
import { applyTheme, calculateModalPosition } from '../utils/commonUtils';
import { ThemeMode, BadgeType, DEFAULT_WINDOW_TITLE, DEFAULT_NOTIFICATION_FETCH_COUNT } from '../utils/constants';
...
const styles = computed(() => applyTheme(props?.darkMode ? props?.theme?.dark : props?.theme?.light,
...
watch(showNotifications, () => {
  if (showNotifications.value) document.addEventListener('click', handleOutsideClick);
